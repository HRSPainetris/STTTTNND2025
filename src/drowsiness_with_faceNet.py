...
COUNTER_NON_FACE_THRES_TAP_TRUNG = 100 # So frame lien tuc khong co khuon mat de canh bao TAP TRUNG
COUNTER_NON_FACE_THRES_RESET = 300 # So frame lien tuc khong co khuon mat de reset he thong
...
confidence_thres = 0.5 # The score for decide the human or not
print("[INFO] loading face detector model...")
prototxtPath = os.path.join(CWD_PATH,"face_detector", "deploy.prototxt")
weightsPath = os.path.join(CWD_PATH,"face_detector", "res10_300x300_ssd_iter_140000.caffemodel")
...
###############################################################
## TODO 10: 2 IP CAMERA VIDEOS (L+R) + 1 USB CAMERA (DRIVER) ##
###############################################################

#########################################################
## TODO 11: 2 IP CAMERAS (L+R) + 1 USB CAMERA (DRIVER) ##
#########################################################
...
##########################################################
## TODO 12: 2 USB CAMERAS (L+R) + 1 USB CAMERA (DRIVER) ##
##########################################################
...
###################################################
##         TODO 15: INTERFACE WITH PX4           ##
###################################################
# Connect to the PX4 to get the GPS data

###################################################
##         TODO 16: INPUT FROM KEYBOARD          ##
###################################################
...
def capture_and_convert_driver_frame():
    ...
    driver_frame = imutils.resize(driver_frame, width = w_process_size)    #450, cang nho xu ly cang nhanh
...
def face_detect_with_faceNet():
    ...
    # Make model input using rgb_frame
    blob = cv2.dnn.blobFromImage(driver_frame, 1.0, (300, 300),
                                (104.0, 177.0, 123.0))
    ...
    return driver_frame, gray, rects
...
def drowsiness_detection():
    ...
    if len(rects) > 0:
        ...
        for rect in rects:
            ...
            dlib_rect = dlib.rectangle(rect[0], rect[1], rect[2], rect[3])
